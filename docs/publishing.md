...
`category`는 선택 필드이며 생략하면 `other`로 분류됩니다. 가져오기 스크립트도 이 값을 보존하며, 오타나 등록되지 않은 값은 거부합니다.
...
카테고리 선택은 `/post?category=development`처럼 주소에 유지됩니다. 기존 백엔드 글에는 카테고리 필드가 없으므로 현재는 `기타`에서 조회합니다. 공개 글만 카테고리별 개수에 포함됩니다.

상세 목차는 본문의 제목(`#`~`######`, 기존 HTML의 `h1`~`h6`)으로 자동 생성합니다. 제목 단계에 따라 들여쓰고 같은 제목이 반복되어도 각각 다른 이동 주소를 만듭니다. 넓은 화면에서는 오른쪽에 고정되어 현재 읽는 항목을 강조하고, 좁은 화면에서는 본문 위의 접이식 목차로 표시됩니다.
