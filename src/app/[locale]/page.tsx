...
import { getLocalizedRecentPosts } from "@lib/content/localized-posts"
...
export async function generateMetadata({ params }: LocalePageProps) {
    return staticPageMetadata("/", await pageLocale(params))
}
export default async function Home({ params }: LocalePageProps) {
    const locale = await pageLocale(params)
    const en = locale === "en"
    const projects = translateData(originalProjects, locale)
    const posts = await getLocalizedRecentPosts(locale, 3)
    ...
    return <div className={styles.home}>
        <section className={styles.intro}>
            <p className={styles.eyebrow}>{en ? "Basilri Kim · Development notes" : "김바실리의 개발 기록"}</p>
            <h1>{en ? <>Building, running,<br />and writing what I learn.</> : <>만들고, 운영하고,<br />배운 것을 기록합니다.</>}</h1>
            <p className={styles.description}>{en ? "From web development to AI. Problems I have worked through, how I solved them, and the projects I am building." : "웹 개발부터 AI까지. 직접 부딪힌 문제와 해결 과정, 그리고 만들고 있는 프로젝트를 소개합니다."}</p>
            <div className={styles.links}><Link href="/post">{en ? "Read the posts ↗" : "개발 글 읽기 ↗"}</Link><Link href="/introduce">{en ? "About me" : "개발자 소개"}</Link></div>
        </section>
        ...
        <section aria-labelledby="featured-projects" className={styles.section}>
            <div className={styles.sectionHeading}><h2 id="featured-projects">{en ? "What I am building" : "만들고 있는 것들"}</h2><Link href="/projects">{en ? "All projects →" : "모든 프로젝트 →"}</Link></div>
            <div className={styles.projects}>{featured.map((project) => <article key={project.id} className={styles.project}>
                ...
                <TrackedLink className={styles.projectLink} href={project.url} eventName="project_open" label={project.url}>{en ? "Explore the project →" : "프로젝트 살펴보기 →"}</TrackedLink>
            </article>)}</div>
        </section>
        <div className={styles.archive}><p>{en ? "Past talks and learning materials are collected here too." : "지난 발표와 학습 자료도 모아두었습니다."}</p><Link href="/research">{en ? "Research & learning →" : "연구·학습 기록 →"}</Link><Link href="/feed.xml">{en ? "Subscribe via RSS →" : "RSS 구독 →"}</Link></div>
    </div>
}
