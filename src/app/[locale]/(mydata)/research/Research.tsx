"use client"

import { useLocale } from "@lib/i18n/context"
import { translateData, translateText } from "@lib/i18n/translate"
import { ReactElement } from "react"
import Link from "@components/ui/LocaleLink"
import classNames from "classnames"
import LineBasic from "@components/atom/LineBasic"
import TextBasic from "@components/atom/TextBasic"
import Wrapper from "@components/layout/Wrapper"
import originalData from "@lib/json/seminar.json"
import { useCoreStore } from "@lib/stores/store"
import { formatArchiveDate } from "@lib/i18n/config"
import { formatDate } from "@lib/utils/common"
import styles from "@styles/pages/career.module.scss"

interface IResearch {
    id: number
    title: string
    subTitle?: string
    date: string
    url?: string
}

const Research = (): ReactElement => {
    const locale = useLocale()
    const t = (text: string) => translateText(text, locale)
    const seminarData = translateData(originalData, locale)
    const { darkMode } = useCoreStore()

    const researchList: IResearch[] = seminarData

    return (
        <Wrapper>
            <div className={styles.wholeWrapper}>
                <div className={styles.seminarAndRND}>
                    <TextBasic as="h1" size="xxx-large" bold="bold">
                        {t("Research | 연구개발 & 교육 & 보고")}
                    </TextBasic>
                    <div className={styles.dotWrapper}>
                        <span className={styles.red}>*</span>
                        <TextBasic size={"small"} bold={"normal"}>
                            {t(": 외부 링크로 연결됩니다.")}
                        </TextBasic>
                    </div>
                    <br />
                    <LineBasic />
                    <br />
                    {researchList.map((seminar) => (
                        <Link
                            key={seminar.id + "_" + seminar.date + "_" + seminar.title}
                            href={seminar.url ?? "#"}
                            target={seminar.url ? "_blank" : ""}
                            scroll={false}
                        >
                            <div
                                className={classNames(
                                    styles.seminarBlock,
                                    seminar.url && styles.active,
                                    darkMode && styles.dark,
                                )}
                                style={{
                                    cursor: seminar.url ? "pointer" : "default",
                                }}
                            >
                                <TextBasic as="h2" className={styles.wrapper} size="x-large" bold="bold">
                                    {seminar.url && <span className={styles.red}>*</span>}
                                    {seminar.title}
                                </TextBasic>
                                {seminar.subTitle && (
                                    <div className={styles.subTitle}>
                                        <TextBasic className={styles.subTitle} size="small" bold="bold">
                                            {`/ ${seminar.subTitle}`}
                                        </TextBasic>
                                    </div>
                                )}
                                <TextBasic size="x-small">{locale === "en" ? formatArchiveDate(seminar.date) : formatDate(seminar.date)}</TextBasic>
                            </div>
                        </Link>
                    ))}
                </div>
            </div>
        </Wrapper>
    )
}

export default Research
