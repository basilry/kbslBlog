"use client"

import { useLocale } from "@lib/i18n/context"
import { ReactElement, useState } from "react"
import { Pagination } from "swiper/modules"
import { Swiper, SwiperSlide } from "swiper/react"
import ModalBasic from "@components/modal/ModalBasic"
import styles from "@styles/pages/projectsPics.module.scss"
import "swiper/css"
import "swiper/css/pagination"

interface IPicsTemplate {
    projectTitle: string
    filePath: string
    domainName: string
    fileNums: number
}

const PicsTemplate = ({ projectTitle, filePath, domainName, fileNums = 6 }: IPicsTemplate): ReactElement => {
    const en = useLocale() === "en"
    const [open, setOpen] = useState({ doOpen: false, idx: -1 })

    return (
        <div className={styles.picsWrapper}>
            <div className={styles.picsBlock}>
                <Swiper pagination={true} modules={[Pagination]} className="mySwiper">
                    {new Array(fileNums).fill(0).map((_, idx) => (
                        <SwiperSlide
                            key={idx}
                            className={styles.block}
                            onClick={(): void => setOpen({ doOpen: true, idx: idx + 1 })}
                        >
                            <img
                                className={styles.projectImages}
                                src={`/${filePath}/${domainName}${idx + 1}.png`}
                                alt={`${projectTitle} ${en ? "screenshot" : "화면"} ${idx + 1}`}
                                loading="lazy"
                                decoding="async"
                            />
                        </SwiperSlide>
                    ))}
                </Swiper>
            </div>
            <ModalBasic
                isOpen={open.doOpen}
                onClose={(): void => setOpen({ doOpen: false, idx: -1 })}
                title={en ? "Screenshot detail" : "상세화면"}
            >
                <img className={styles.modalImages} src={`/${filePath}/${domainName}${open.idx}.png`} alt={`${projectTitle} ${en ? "screenshot" : "화면"} ${open.idx}`} />
            </ModalBasic>
        </div>
    )
}

export default PicsTemplate
