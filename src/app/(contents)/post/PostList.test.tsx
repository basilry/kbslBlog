...
describe("search list navigation", () => {
    it("submits a new search from page one and preserves query/category in previous and next links", () => {
        ...
        expect(links).toContain("/search?q=AI+%26+%EB%B3%B4%EC%95%88&category=ai-agents")
        expect(links).toContain("/search?q=AI+%26+%EB%B3%B4%EC%95%88&category=ai-agents&page=3")
        expect(links).toContain("/search?q=AI+%26+%EB%B3%B4%EC%95%88&category=development")
        expect(links).toContain("/search?category=ai-agents")
    })
    ...
    it("retains ordinary list pagination outside search and has no legacy API warning in static search", () => {
        ...
        expect(list).toContain('href="/post?category=ai-agents&amp;page=3"')
