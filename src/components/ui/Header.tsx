...
export default function Header() {
    ...
    return <header className={classNames(styles.headerWrapper, darkMode && styles.darkMode)}>
        ...
        <div className={styles.rightWrapper}>
            <DarkModeBasic />
            {loginState ? <button type="button" className={styles.authButton} onClick={() => { initialize(); router.refresh(); toastCall("로그아웃 되었습니다.", "success") }}>로그아웃</button>
                : <Link href="/login" className={styles.authButton}>로그인</Link>}
        </div>
    </header>
}
